...
// Cloudflare Worker URL（主方案，如不可用则自动降级）
const WORKER_URL = import.meta.env.VITE_WORKER_URL || '';

// 百度翻译 API 配置（降级方案：直接调用 + CORS 代理）
...
// 生成百度翻译 API 签名参数
function buildBaiduParams(text: string, from: string, to: string): URLSearchParams {
...
// 降级方案：直接调用百度 API + CORS 代理
async function fetchDirectBaidu(params: URLSearchParams): Promise<any> {
  const body = params.toString();

  // 策略1: 直接调用（localhost 开发环境可行）
  try {
  ...
  try {
    ...
    if (WORKER_URL) {
      try {
        ...
        console.warn('Worker 不可用，降级到直接调用百度 API');
      }
    }

    // === 方案2: 直接调用百度 API（含 CORS 代理降级）===
    const params = buildBaiduParams(text, from, to);
    const data = await fetchDirectBaidu(params);
